    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {filtered.map((post) => (
          ...
          >
            {post.image && (
            <div className={`aspect-video w-full overflow-hidden bg-muted flex items-center justify-center ${post.image.endsWith('.svg') ? 'p-4' : ''}`}>
              <img
                src={post.image}
                alt={post.title}
                className={`w-full h-full transition-transform duration-300 group-hover:scale-105 ${
                  post.image.endsWith('.svg') ? 'object-contain' : 'object-cover'
                }`}
              />
            </div>
          )}
